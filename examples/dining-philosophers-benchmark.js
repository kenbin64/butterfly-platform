...
function benchmark(name, iterations, fn) {
    ...
    const avgNs = (totalMs * 1000000) / iterations;
...
const game = gameengine_1.GameSubstrate.create();
...
const forkCheck = benchmark("Manifold fork check (drill to state)", ITERATIONS, () => {
    game.philosopher(0).drill("fork_left").value;
    game.philosopher(0).drill("fork_right").value;
});
...
const simGame = gameengine_1.GameSubstrate.create();
...
const waveGame = gameengine_1.GameSubstrate.create();
...
const playerWave = waveGame.drillWave(["entities", "player", "health", "current"]);
...
const interference = waveGame.interference(["entities", "player"], ["entities", "enemy_1"]);
...
const cascadeGame = gameengine_1.GameSubstrate.create();
...
const cascadeBench = benchmark("Helical cascade turn (data propagation)", 50000, () => {
...
// Discrete: 7 pairs
const discreteHelix = new flow_1.HelicalCascade();
...
const continuousDrill = new flow_1.ContinuousDrill(1000);
...
const contSample = benchmark("Continuous sample(θ)", 100000, () => {
    continuousDrill.sample(Math.random() * 10);
});
const contSmooth = benchmark("Continuous sampleSmooth(θ)", 10000, () => {
...
const secureDrill = new flow_1.OilRigDrill();
const hackerDrill = new flow_1.OilRigDrill(); // Different random seed
...
const secureBench = benchmark("Secure OilRigDrill.sample(θ)", 100000, () => {
...
const diamondDrill = new flow_1.DiamondDrill();
...
const diamondBench = benchmark("DiamondDrill.sample(θ)", 100000, () => {
...
const wfGame = gameengine_1.GameSubstrate.create();
...
const fullRoundtrip = benchmark("Full manifold round-trip (encode→verify→materialise)", 10000, () => {
    wfGame.encodeToWaveform(testPayload);
    wfGame.verifyWaveform();
    wfGame.materialiseJSON();
});
const jsonFullRoundtrip = benchmark("Full JSON round-trip (stringify→parse→validate)", 10000, () => {
