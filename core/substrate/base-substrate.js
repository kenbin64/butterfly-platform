...
exports.SimulationSubstrate = exports.BaseSubstrate = exports.HELIX_FIELD = exports.HELIX = exports.DIMENSIONAL_NAMES = void 0;
...
exports.HELIX = Object.freeze([
    new saddle_1.SaddleForm(0), // Section 0: Void   → Point
    new saddle_1.SaddleForm(HALF_PI), // Section 1: Point  → Line
    new saddle_1.SaddleForm(Math.PI), // Section 2: Line   → Width
    new saddle_1.SaddleForm(3 * HALF_PI), // Section 3: Width  → Plane
    new saddle_1.SaddleForm(4 * HALF_PI), // Section 4: Plane  → Volume
    new saddle_1.SaddleForm(5 * HALF_PI), // Section 5: Volume → Whole
    new saddle_1.SaddleForm(6 * HALF_PI), // Section 6: Whole  → Object (collapse to higher dim)
]);
/** SaddleField backed by the helix — each cell governs its own region of the manifold. */
exports.HELIX_FIELD = (() => {
    let field = new saddlefield_1.SaddleField();
    for (let i = 0; i < 7; i++)
        field = field.place([i, 0], exports.HELIX[i]);
    return field;
})();
...
for (let _i = 0; _i < 7; _i++) {
    _HC[_i] = Math.cos(exports.HELIX[_i].orientation);
    _HS[_i] = Math.sin(exports.HELIX[_i].orientation);
}
/**
...
class BaseSubstrate {
    constructor(config, initialValue) {
        ...
        this._lastEncodedData = undefined; // last input reference
        this._lastEncodedStr = null; // serialised form of last input
        this._cachedEncoding = null; // result of last encode
        ...
        this._lastMaterialisedWf = null; // encoding ref at last materialise
        this._cachedBuffer = null; // pre-built byte buffer
        this._cachedJSON = undefined; // pre-parsed JSON object
        this._cachedJSONReady = false; // guards undefined vs unparsed
     ...
     */
    readManifold(x, y, sectionIndex = 0) {
    ...
    setPath(key, value) {
        if (typeof value === "number") {
            this._pathTable[key] = (0, path_expressions_1.discoverPath)(value, key.length % 7);
        }
        else {
            this._pathTable[key] = (0, path_expressions_1.stringToPathExprs)(value);
        }
    }
    /**
    ...
    getPath(key) {
        ...
        if (Array.isArray(expr)) {
            return (0, path_expressions_1.pathExprsToString)(expr);
        }
        return (0, path_expressions_1.evaluatePath)(expr);
    }
    /**
    ...
    _encodeBytesRaw(raw) {
        ...
        const encoding = { xValues, yValues, checksum, dimension: 0 };
     ...
     */
    materialisePixels(width, height) {
     ...
     */
    materialiseAudioSamples(length = 44100) {
     ...
     */
    dimensionalName(sectionIndex) {
        return exports.DIMENSIONAL_NAMES[((sectionIndex % 7) + 7) % 7];
    }
    // ─── HARMONIC COMPACTION ─────────────────────────────────────────────────
    /**
     ...
     */
    compactWaveform(blockSize = 4096, maxHarmonics = 64) {
        const enc = this._storedWaveform();
        if (!enc)
            return null;
        return (0, harmonic_compaction_1.compactHarmonics)(enc.yValues, blockSize, maxHarmonics);
    }
    /**
     ...
     */
    expandFromHarmonics(manifold) {
        return (0, harmonic_compaction_1.expandHarmonics)(manifold);
    }
    /** Convenience: storage size of a harmonic manifold in bytes. */
    harmonicStorageBytes(manifold) {
        return (0, harmonic_compaction_1.harmonicStorageBytes)(manifold);
    }
    /** Mean Squared Error between original yValues and reconstructed. */
    harmonicMSE(original, reconstructed) {
        return (0, harmonic_compaction_1.computeMSE)(original, reconstructed);
    }
    /** Peak Signal-to-Noise Ratio (dB) between original and reconstructed. */
    harmonicPSNR(original, reconstructed) {
        return (0, harmonic_compaction_1.computePSNR)(original, reconstructed);
    }
    // ─── PROTECTED HELPERS ─────────────────────────────────────────────────────
    /**
...
BaseSubstrate._enc = new TextEncoder();
BaseSubstrate._dec = new TextDecoder();
/** Re-export for external use. */
BaseSubstrate.evaluatePath = path_expressions_1.evaluatePath;
BaseSubstrate.discoverPath = path_expressions_1.discoverPath;
/**
 * SimulationSubstrate
 * -------------------
