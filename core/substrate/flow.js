...
exports.StaticFlow = exports.ContinuousDrill = exports.OilRigDrill = exports.DiamondDrill = exports.DrillSection = exports.HelicalCascade = exports.SaddlePair = void 0;
 ...
 */
class HelicalCascade {
    constructor(initialState) {
        this.TURN_KEYS = [2, 4, 6];
        this._observers = new Set();
        this.pairs = Array.from({ length: 7 }, (_, i) => new SaddlePair(i + 1, initialState?.[i] ?? 0));
    }
    ...
    state() { return this.pairs.map(p => p.rotation); }
...
exports.HelicalCascade = HelicalCascade;
// ═══════════════════════════════════════════════════════════════════════════
 ...
 */
class DrillSection {
    constructor(index, amplitude = 1, wavelength = 1) {
        // DINING PHILOSOPHERS - Fork state for synchronization
        this._leftFork = false; // Coupling to previous section
        this._rightFork = false; // Coupling to next section
        this._eating = false; // Currently accessing data
        this.index = index;
...
exports.DrillSection = DrillSection;
/**
...
class DiamondDrill {
    constructor(seed) {
        this._shaftRotation = 0;
        // Dining Philosophers: 7 forks between 7 philosophers (circular)
        this._forks = [false, false, false, false, false, false, false];
    ...
    get rotation() { return this._shaftRotation; }
    /** Get the complete waveform across all sections */
    getWaveform(resolution = 100) {
        const wave = [];
...
exports.DiamondDrill = DiamondDrill;
// ═══════════════════════════════════════════════════════════════════════════
...
class OilRigDrill {
     ...
     */
    addHarmonic(amplitude, phase, frequency = 1) {
    ...
    encode(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            this.addHarmonic(data[i], -this._seedHash(i), // Pre-scramble phase so sample() unscrambles it
            i + 1);
        }
    }
    /**
    ...
    get harmonicCount() { return this._harmonics.length; }
    /**
     ...
     */
    findInflections(startTheta, endTheta, resolution = 100) {
        const inflections = [];
     ...
     */
    getSections(startTheta, endTheta) {
...
exports.OilRigDrill = OilRigDrill;
/**
...
class ContinuousDrill {
    ...
    sampleSmooth(theta) {
        ...
        for (const k of this._distortions.keys()) {
            if (k <= theta && (lower === undefined || k > lower))
                lower = k;
            if (k >= theta && (upper === undefined || k < upper))
                upper = k;
        }
        if (lower === undefined && upper === undefined)
            return 0;
        if (lower === undefined)
            return this._distortions.get(upper);
        if (upper === undefined)
            return this._distortions.get(lower);
        if (lower === upper)
            return this._distortions.get(lower);
    ...
    get storageSize() { return this._distortions.size; }
...
exports.ContinuousDrill = ContinuousDrill;
/**
 * StaticFlow
 * ----------
 ...
 */
class StaticFlow {
    constructor(origin, direction, magnitude) {
        this.origin = origin;
        const len = (0, ops_1.mag)(direction);
        this.direction = len === 0 ? direction.map(() => 0) : (0, ops_1.scale)(direction, 1 / len);
     ...
     */
    project(t) {
        return (0, ops_1.add)(this.origin, (0, ops_1.scale)(this.direction, this.magnitude * t));
    }
    /**
     * sample(n)
     * ---------
