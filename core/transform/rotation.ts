...
export const planeRotation = (n: number, i: number, j: number, theta: number): Matrix => {
    if (i === j) {
...
export const rotateVector = (R: Matrix, v: VecN): VecN => {
    const out: VecN = [];
...
export const rotationBetween = (v: VecN, w: VecN): Matrix => {
    ...
    // Normalize.
    const vn: VecN = v.map(x => x / magV);
    const wn: VecN = w.map(x => x / magW);
    ...
    // Normalize v2.
    let v2norm = 0;
    for (let i = 0; i < n; i++) v2norm += v2[i] * v2[i];
    v2norm = Math.sqrt(v2norm);
    for (let i = 0; i < n; i++) v2[i] /= v2norm;
