    ...
    entities.forEach(entity => {
      ...
      if (settings) {
        if (newPosition.x < 0 || newPosition.x > settings.width) {
          ...
          newPosition.x = Math.max(0, Math.min(settings.width as number, newPosition.x));
        }
        if (newPosition.y < 0 || newPosition.y > settings.height) {
          entity.velocity.y *= -0.8;
          newPosition.y = Math.max(0, Math.min(settings.height as number, newPosition.y));
        }
      }
      ...
      if (entity.type === "enemy") {
        ...
        if (entity.health <= 0) {
          ...
          const currentScore = (this.dimensionalState.drill("game", "score").value as number) || 0;
          this.dimensionalState.drill("game", "score").value = currentScore + 10;
        }
      }
