...
import { PhysicsSubstrate, Vec3 } from "../app/src/engine/physicsengine";
import { VideoSubstrate, RGBA } from "../app/src/engine/videoEngine";
import { AudioSubstrate } from "../app/src/engine/audeoEngine";
import { GameSubstrate } from "../app/src/engine/gameengine";
import { AutonomousSubstrate } from "../app/src/engine/atomomousEngine";
...
describe("Manifold Compliance: PhysicsSubstrate", () => {
  ...
  test("drill() provides O(1) body access", () => {
    ...
    const pos = player.drill<Vec3>("position").value;
  ...
  test("manifold is deterministic - same drill = same result", () => {
    ...
    const mass1 = physics.body("test").drill<number>("mass").value;
    const mass2 = physics.body("test").drill<number>("mass").value;
  ...
  test("nested coordinates are drillable", () => {
    ...
    const gravity = physics.drill<Vec3>("world", "gravity").value;
...
describe("Manifold Compliance: VideoSubstrate", () => {
  let video: VideoSubstrate;

  beforeEach(() => {
    video = VideoSubstrate.create();
  ...
  test("metadata is drillable", () => {
    expect(video.drill<number>("meta", "width").value).toBe(100);
    expect(video.drill<number>("meta", "height").value).toBe(100);
  });
});

describe("Manifold Compliance: AudioSubstrate", () => {
  let audio: AudioSubstrate;

  beforeEach(() => {
    audio = AudioSubstrate.create();
  });
  ...
  test("track access is O(1)", () => {
    ...
    const drumsVol = audio.track("drums").drill<number>("volume").value;
  ...
  test("BPM is a drillable coordinate", () => {
    ...
    expect(audio.drill<number>("master", "bpm").value).toBe(140);
  });

  test("effects are drillable", () => {
    audio.addEffect("reverb_1", "reverb", { wet: 0.5, decay: 2.0 });
    
    const wet = audio.drill<number>("effects", "reverb_1", "params", "wet").value;
    expect(wet).toBe(0.5);
  });
});

describe("Manifold Compliance: GameSubstrate", () => {
  let game: GameSubstrate;

  beforeEach(() => {
    game = GameSubstrate.create();
  });
  ...
  test("entity access is O(1)", () => {
    ...
    expect(player.drill<boolean>("active").value).toBe(true);
  });
  ...
  test("component drilling works", () => {
    ...
    const x = game.component("player", "transform").drill<number>("x").value;
    expect(x).toBe(10);
  });
  ...
  test("game state is drillable", () => {
    ...
    expect(game.getState<number>("score")).toBe(100);
    expect(game.drill<number>("state", "level").value).toBe(5);
  });
});
...
describe("Dining Philosophers Synchronization", () => {
  ...
  test("philosophers are drillable coordinates", () => {
    ...
    expect(p0.drill<boolean>("thinking").value).toBe(true);
    expect(p4.drill<boolean>("eating").value).toBe(false);
  });

  test("philosopher state is navigated, not computed", () => {
    // Drill directly to state - no lock computation
    expect(game.drill<boolean>("sync", "philosophers", "p_2", "eating").value).toBe(false);
    expect(game.drill<boolean>("sync", "philosophers", "p_2", "fork_left").value).toBe(true);
  });
  ...
  test("tryEat acquires forks and sets eating state", () => {
    ...
    expect(game.philosopher(0).drill<boolean>("thinking").value).toBe(false);
  });
  ...
  test("finishEating releases forks", () => {
    ...
    expect(game.philosopher(0).drill<boolean>("thinking").value).toBe(true);
  ...
  test("fork state is drillable - O(1) check", () => {
    ...
    const fork = game.philosopher(0).drill<boolean>("fork_right");
  ...
  test("synchronization state persists in manifold", () => {
    ...
    const eating0 = game.drill<boolean>("sync", "philosophers", "p_0", "eating").value;
    const eating1 = game.drill<boolean>("sync", "philosophers", "p_1", "eating").value;
    const eating2 = game.drill<boolean>("sync", "philosophers", "p_2", "eating").value;
