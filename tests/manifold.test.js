...
const ops_1 = require("../core/ops");
const saddlefield_1 = require("../core/substrate/saddlefield");
const saddlenetwork_1 = require("../core/substrate/saddlenetwork");
const extractor_1 = require("../core/substrate/extractor");
const interpreter_1 = require("../core/substrate/interpreter");
const renderer_1 = require("../core/substrate/renderer");
const agent_1 = require("../core/substrate/agent");
const interaction_1 = require("../core/substrate/interaction");
const manifold_1 = require("../core/manifold/manifold");
const basis_1 = require("../core/transform/basis");
const observer_1 = require("../core/manifold/observer");
const eps = 1e-9;
...
describe("SaddleForm", () => {
    test("valueAt(0°): z=xy", () => {
        const f = new saddle_1.SaddleForm(0);
    ...
    test("valueAt(90°): z=-xy  (lock rotated)", () => {
        const f = new saddle_1.SaddleForm(Math.PI / 2);
    ...
    test("zeroDirections are orthogonal", () => {
        const f = new saddle_1.SaddleForm(Math.PI / 4);
    ...
    test("zeroDirections are unit vectors", () => {
        const f = new saddle_1.SaddleForm(0.7);
        const [d1, d2] = f.zeroDirections();
    ...
    test("along a zeroDirection, z is identically 0", () => {
        const f = new saddle_1.SaddleForm(0.5);
...
describe("Lock-and-key (90° rotation)", () => {
    ...
    test("four locks (360°) = identity", () => {
        const f = new saddle_1.SaddleForm(0.3);
...
describe("SaddlePair", () => {
    test("secondary is 90° ahead of primary", () => {
        const pair = new saddle_1.SaddlePair(0.2);
    ...
    test("coupling at 0° is non-zero (aligned forms)", () => {
        const pair = new saddle_1.SaddlePair(0);
...
describe("SaddleField", () => {
    const form0 = new saddle_1.SaddleForm(0);
    const form90 = new saddle_1.SaddleForm(Math.PI / 2);
    const field = new saddlefield_1.SaddleField().place([0, 0], form0);
    ...
    test("two-cell field: nearest cell governs each region", () => {
        const f2 = new saddlefield_1.SaddleField()
            .place([0, 0], form0)
            .place([10, 0], form90);
...
describe("SaddleNetwork", () => {
    test("write then read round-trips a value", () => {
        const net = new saddlenetwork_1.SaddleNetwork().addNode("A");
        net.write("A", 42);
        expect(net.read("A")).toBe(42);
    });
    test("step propagates output from source to downstream node", () => {
        const net = new saddlenetwork_1.SaddleNetwork()
            .addNode("src")
            .addNode("dst")
            .connect("src", "dst");
        net.write("src", 3);
    ...
    test("node with no incoming edges keeps its written value after step", () => {
        const net = new saddlenetwork_1.SaddleNetwork().addNode("isolated");
    ...
    test("lock (0° aligned) couples positively; key (90° apart) couples negatively", () => {
        ...
        const locked1 = new saddle_1.SaddleForm(0);
        const locked2 = new saddle_1.SaddleForm(0);
        const lockCoupling = (0, ops_1.dot)(locked1.queryBroadcast(y0, xs), locked2.queryBroadcast(y0, xs));
        expect(lockCoupling).toBeGreaterThan(0);
        // One form at 0°, one at 90° — KEY TURNED: profiles are negatives → dot < 0
        const key = new saddle_1.SaddleForm(Math.PI / 2);
        const keyCoupling = (0, ops_1.dot)(locked1.queryBroadcast(y0, xs), key.queryBroadcast(y0, xs));
...
// ── 6. Extractor ─────────────────────────────────────────────────────────────
describe("Extractor", () => {
    const form = new saddle_1.SaddleForm(0);
    const field = new saddlefield_1.SaddleField().place([0, 0], form);
    test("extractAlongPath finds zero crossing on y-axis (x=0 → z=0)", () => {
        ...
        const path = linspace(-2, 2, 41).map(x => [x, 1]);
        const result = (0, extractor_1.extractAlongPath)(field, path);
        expect(result.summary.zeroCount).toBeGreaterThan(0);
    });
    test("extractAlongPath values match scalarAt", () => {
        const path = [[1, 1], [2, 1], [3, 1]];
        const result = (0, extractor_1.extractAlongPath)(field, path);
        result.values.forEach((z, i) => expect(near(z, field.scalarAt(path[i]))).toBe(true));
    });
    test("summary totalVariation > 0 on a non-flat path", () => {
        const path = linspace(-2, 2, 20).map(x => [x, 1]);
        const result = (0, extractor_1.extractAlongPath)(field, path);
        expect(result.summary.totalVariation).toBeGreaterThan(0);
    });
    test("extractNetwork returns an entry per node", () => {
        const net = new saddlenetwork_1.SaddleNetwork().addNode("p").addNode("q");
        net.write("p", 5);
        net.write("q", -3);
        const out = (0, extractor_1.extractNetwork)(net);
        expect(out["p"]).toBe(5);
        expect(out["q"]).toBe(-3);
    });
    ...
    test("flat path along y=0 produces no sign-change zero crossings", () => {
        ...
        const path = linspace(-2, 2, 20).map(x => [x, 0]);
        const result = (0, extractor_1.extractAlongPath)(field, path);
...
describe("Interpreter", () => {
    const zeroCrossing = { kind: "zero", position: [0, 0, 0], value: 0 };
    const turningPt = { kind: "turning", position: [1, 1, 2], value: 2 };
    const inflectionPt = { kind: "inflection", position: [2, 2, 1], value: 1 };
    const saddlePt = { kind: "saddle", position: [0, 0, 0], value: 0 };
    test("zero → boundary", () => {
        const [v] = (0, interpreter_1.interpret)([zeroCrossing]);
        expect(v.kind).toBe("boundary");
    });
    test("turning → scalar with value preserved", () => {
        const [v] = (0, interpreter_1.interpret)([turningPt]);
        expect(v.kind).toBe("scalar");
        expect(near(v.value, 2)).toBe(true);
    });
    test("inflection → transition", () => {
        const [v] = (0, interpreter_1.interpret)([inflectionPt]);
        expect(v.kind).toBe("transition");
    });
    test("saddle → origin", () => {
        const [v] = (0, interpreter_1.interpret)([saddlePt]);
        expect(v.kind).toBe("origin");
    });
    test("reduce: sum of scalars", () => {
        const vals = (0, interpreter_1.interpret)([turningPt, { kind: "turning", position: [3, 3, 3], value: 3 }]);
        expect(near((0, interpreter_1.reduce)(vals, "sum"), 5)).toBe(true);
    });
    test("reduce: mean of scalars", () => {
        const vals = (0, interpreter_1.interpret)([turningPt, { kind: "turning", position: [3, 3, 3], value: 3 }]);
        expect(near((0, interpreter_1.reduce)(vals, "mean"), 2.5)).toBe(true);
    });
    // DISPROOF ATTEMPT: non-scalar features should not contribute to reduce("sum")
    test("reduce ignores non-scalar features", () => {
        const vals = (0, interpreter_1.interpret)([zeroCrossing, saddlePt, inflectionPt]);
        expect(near((0, interpreter_1.reduce)(vals, "sum"), 0)).toBe(true); // no scalars → 0
    });
});
// ── 8. Renderer ──────────────────────────────────────────────────────────────
describe("Renderer", () => {
    const field = new saddlefield_1.SaddleField().place([0, 0], new saddle_1.SaddleForm(0));
    const xs = linspace(-2, 2, 10);
    const ys = linspace(-2, 2, 8);
    test("sampleField produces correct dimensions", () => {
        const frame = (0, renderer_1.sampleField)(field, xs, ys);
    ...
    test("renderAsAscii produces correct number of lines", () => {
        const frame = (0, renderer_1.sampleField)(field, xs, ys);
        const ascii = (0, renderer_1.renderAsAscii)(frame);
        expect(ascii.split("\n")).toHaveLength(8);
    });
    test("renderAsData produces width*height records", () => {
        const frame = (0, renderer_1.sampleField)(field, xs, ys);
        const data = (0, renderer_1.renderAsData)(frame);
    ...
    test("sample at origin has z=0", () => {
        const frame = (0, renderer_1.sampleField)(field, [0], [0]);
        expect(near(frame.samples[0][0].z, 0)).toBe(true);
    });
});
// ── 9. Agent fix: no perception roundtrip ────────────────────────────────────
describe("Agent perception (bug fix proof)", () => {
    const basis = (0, basis_1.standardBasis)(2);
    const chart = (0, manifold_1.chartFromBasis)(basis);
    const obs = (0, observer_1.observerAtChartOrigin)(chart);
    const A = (0, agent_1.agent)({ ...obs, position: [3, 4] });
    test("perceiveScalar reads at agent position, not origin", () => {
        // Field: f(p) = p[0] * p[1]
        const F = { valueAt: (p) => p[0] * p[1] };
        expect(near((0, agent_1.perceiveScalar)(A, F), 12)).toBe(true); // 3*4 = 12
    });
    test("perceiveVector reads at agent position directly", () => {
        // Field returns the position itself
        const F = { valueAt: (p) => p };
        const v = (0, agent_1.perceiveVector)(A, F);
    ...
    test("two agents at different positions perceive different field values", () => {
        const F = { valueAt: (p) => p[0] * p[1] };
        const A1 = (0, agent_1.agent)({ ...obs, position: [2, 3] });
        const A2 = (0, agent_1.agent)({ ...obs, position: [5, 7] });
        expect((0, agent_1.perceiveScalar)(A1, F)).not.toBeCloseTo((0, agent_1.perceiveScalar)(A2, F), 5);
    });
});
...
describe("applyPairwise (bug fix proof)", () => {
    ...
    const touch = (a, _b) => ({ updated: { value: a.value + 1 } });
    test("both items are updated after one pairwise step", () => {
        const items = [{ value: 0 }, { value: 0 }];
        const result = (0, interaction_1.applyPairwise)(items, touch);
    ...
    test("values are independent — B gets its own update, not A's", () => {
        const asymmetric = (a, _b) => ({ updated: { value: a.value + 10 } });
        const items = [{ value: 1 }, { value: 100 }];
        const result = (0, interaction_1.applyPairwise)(items, asymmetric);
        // A (1) → 1 + 10 = 11
...
describe("HelicalCascade", () => {
    ...
    test("CascadePair.turn() returns true for turn keys", () => {
        const key = new flow_1.SaddlePair(2); // even = turn key
        const notKey = new flow_1.SaddlePair(3); // odd = not turn key
