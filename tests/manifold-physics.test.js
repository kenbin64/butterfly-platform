...
const physics_substrate_1 = require("../app/src/engine/physicsengine/physics-substrate");
describe("Manifold Physics - Primitive Layer", () => {
    let physics;
    beforeEach(() => {
        physics = physics_substrate_1.PhysicsSubstrate.create();
    });
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    test("Physics is deterministic: same initial state → same trajectory", () => {
        ...
        for (let i = 0; i < 50; i++) {
            physics.tick(0.016);
            const pos = physics.body("ball1").drill("position").value;
            run1Pos.push([...pos]);
        }
    ...
    test("Gravity is NOT hardcoded to [0, -9.81, 0]", () => {
        ...
        physics.tick(0.016);
        const pos1 = body.drill("position").value;
